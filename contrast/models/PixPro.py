...
def pixpro_densecl_loss(
    ...
    pos_mask = ((dist_center < pos_ratio).detach()).reshape(
        -1, dist_center.shape[2]
    )  # shape: (bs*S^2, s^2), here S^2==49, boolean values

    # positive pairs similarity (filter out all negative pairs' similarities, and leave with positive pairs')
    logit = torch.masked_select(logit.reshape(-1), pos_mask.reshape(-1)).unsqueeze(
        -1
    )  # shape: (U, 1), U is an integer between [0, bs*S^4]

    # alter pos_mask (get count of positive pairs for each patch in the main branch)
    pos_mask = pos_mask.int().sum(-1)  # shape: (bs*s^2,)

    # negative pairs similarity
    q = q.permute(0, 2, 1)
    q = q.reshape(-1, q.size(2))  # shape: (bs*S^2, C)
    q = torch.einsum(
        "nc,ck->nk",
        [
            torch.repeat_interleave(q, pos_mask, dim=0),
            queue.clone().detach(),
        ],
    )  # shape: (U, K), where K is queue size, and U is an integer between [0, bs*S^4], here S==7
    ...
    return (
            ...
            * torch.repeat_interleave(
                torch.cat(
                    [
                        torch.where(
                            queue_target == v,
                            torch.zeros_like(queue_target),
                            torch.ones_like(queue_target),
                        )
                        .unsqueeze(0)
                        .repeat(dist_center.shape[1], 1)
            ...
            + torch.repeat_interleave(
                torch.cat(
                    [
                        torch.where(queue_target == v, -1e10, 0.0)
                        .unsqueeze(0)
                        .repeat(dist_center.shape[1], 1)
                        for v in target.detach().tolist()
                    ],
        ...
        if class_aware
        else ContrastiveHead(temperature=0.2)(logit, q)
    )
...
class PixPro(nn.Module):
    def __init__(self, base_encoder, args):
        ...
        self.dense_branch_class_aware = args.dense_branch_class_aware
        ...
        #######
        if self.dense_loss_weight > 0.0 and self.dense_loss_func in [
            "PixPro",
            "PixPro_DenseCL",
        ]:
        ...
        ######
        ### create dense-branch buffer
        ######
        if self.dense_loss_weight > 0.0 and self.dense_loss_func == "PixPro_DenseCL":
            if self.head_type == "c4c5":
                self.queue_len_c4 = 50176  # multiple of bs*49 = 2*128*196 = 50176
                self.queue_len_c5 = 50176  # multiple of bs*49 = 2*128*49 = 12544
                feat_dim = 256

                self.register_buffer(
                    "queue_c4", torch.randn(feat_dim, self.queue_len_c4)
                )
                self.register_buffer(
                    "queue_c5", torch.randn(feat_dim, self.queue_len_c5)
                )
                self.queue_c4 = nn.functional.normalize(self.queue_c4, dim=0)
                self.queue_c5 = nn.functional.normalize(self.queue_c5, dim=0)
                self.register_buffer("queue_ptr_c4", torch.zeros(1, dtype=torch.long))
                self.register_buffer("queue_ptr_c5", torch.zeros(1, dtype=torch.long))

                # queue for image class (target)
                if self.dense_branch_class_aware:
                    self.register_buffer(
                        "queue_target_c4", torch.randint(20, 30, (self.queue_len_c4,))
                    )
                    self.register_buffer(
                        "queue_target_c5", torch.randint(20, 30, (self.queue_len_c5,))
                    )
                    self.register_buffer(
                        "queue_target_ptr_c4", torch.zeros(1, dtype=torch.long)
                    )
                    self.register_buffer(
                        "queue_target_ptr_c5", torch.zeros(1, dtype=torch.long)
                    )
            else:
                self.queue_len = 50176  # multiple of bs*49 (or bs*16) = 2*128*49 (2*128*16) = 12544 (4096)
                feat_dim = 256

                self.register_buffer("queue", torch.randn(feat_dim, self.queue_len))
                ...
                if self.dense_branch_class_aware:
                    self.register_buffer(
                        "queue_target", torch.randint(20, 30, (self.queue_len,))
                    )
                    self.register_buffer(
                        "queue_target_ptr", torch.zeros(1, dtype=torch.long)
                    )
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue_c4(self, keys):
        # gather keys before updating queue
        keys = concat_all_gather(keys)

        batch_size = keys.shape[0]

        ptr = int(self.queue_ptr_c4)
        assert self.queue_len_c4 % batch_size == 0  # for simplicity

        # replace the keys at ptr (dequeue and enqueue)
        self.queue_c4[:, ptr : ptr + batch_size] = keys.transpose(0, 1)
        ptr = (ptr + batch_size) % self.queue_len_c4  # move pointer

        self.queue_ptr_c4[0] = ptr

    @torch.no_grad()
    def _dequeue_and_enqueue_c5(self, keys):
        # gather keys before updating queue
        keys = concat_all_gather(keys)

        batch_size = keys.shape[0]

        ptr = int(self.queue_ptr_c5)
        assert self.queue_len_c5 % batch_size == 0  # for simplicity

        # replace the keys at ptr (dequeue and enqueue)
        self.queue_c5[:, ptr : ptr + batch_size] = keys.transpose(0, 1)
        ptr = (ptr + batch_size) % self.queue_len_c5  # move pointer

        self.queue_ptr_c5[0] = ptr

    @torch.no_grad()
    def _dequeue_and_enqueue_target(self, keys):
        # gather keys before updating queue
        keys = concat_all_gather(keys)

        batch_size = keys.shape[0]

        ptr = int(self.queue_target_ptr)
        assert self.queue_len % batch_size == 0  # for simplicity

        # replace the keys at ptr (dequeue and enqueue)
        self.queue_target[ptr : ptr + batch_size] = keys
        ptr = (ptr + batch_size) % self.queue_len  # move pointer

        self.queue_target_ptr[0] = ptr

    @torch.no_grad()
    def _dequeue_and_enqueue_target_c4(self, keys):
        # gather keys before updating queue
        keys = concat_all_gather(keys)

        batch_size = keys.shape[0]

        ptr = int(self.queue_target_ptr_c4)
        assert self.queue_len_c4 % batch_size == 0  # for simplicity

        # replace the keys at ptr (dequeue and enqueue)
        self.queue_target_c4[ptr : ptr + batch_size] = keys
        ptr = (ptr + batch_size) % self.queue_len_c4  # move pointer

        self.queue_target_ptr_c4[0] = ptr

    @torch.no_grad()
    def _dequeue_and_enqueue_target_c5(self, keys):
        # gather keys before updating queue
        keys = concat_all_gather(keys)

        batch_size = keys.shape[0]

        ptr = int(self.queue_target_ptr_c5)
        assert self.queue_len_c5 % batch_size == 0  # for simplicity

        # replace the keys at ptr (dequeue and enqueue)
        self.queue_target_c5[ptr : ptr + batch_size] = keys
        ptr = (ptr + batch_size) % self.queue_len_c5  # move pointer

        self.queue_target_ptr_c5[0] = ptr
    ...
    def forward(self, *data):
        ...
        if self.aug_type in ["SimCLR", "MOCOv2"]:
            assert len(data) == 3, DATA_INCOMPATIBLE_ERR
            (im_1, im_2, target) = data
        else:
            if self.dense_branch_class_aware or self.instance_branch_class_aware:
                assert len(data) == 5, DATA_INCOMPATIBLE_ERR
                (im_1, im_2, coord1, coord2, target) = data
            elif (
                self.instance_loss_weight > 0.0 and self.instance_loss_func == "RotPred"
            ):
        ...
        if self.dense_loss_weight > 0.0:
            if self.dense_loss_func == "PixPro_DenseCL":
                if self.head_type == "c4c5":
                    loss_dense = 0.5 * (
                        pixpro_densecl_loss(
                            pred_1_c4,
                            ...
                            self.queue_c4,
                            self.dense_branch_class_aware,
                            queue_target=self.queue_target_c4
                            if self.dense_branch_class_aware
                            else None,
                            target=target if self.dense_branch_class_aware else None,
                        )
                        + pixpro_densecl_loss(
                            pred_2_c4,
                            proj_1_ng_c4,
                        ...
                        pixpro_densecl_loss(
                            ...
                            self.queue_c5,
                            self.dense_branch_class_aware,
                            queue_target=self.queue_target_c5
                            if self.dense_branch_class_aware
                            else None,
                            target=target if self.dense_branch_class_aware else None,
                        )
                    ...
                    # enqueue and dequeue two queues
                    self._dequeue_and_enqueue_c4(
                        proj_1_ng_c4.permute(0, 2, 3, 1).reshape(
                            -1, proj_1_ng_c4.shape[1]
                        )
                    )  # proj_1_ng_c4.shape[1] is channel dimension, the resultant shape is (bs*S^2, C)
                    if self.dense_branch_class_aware:
                        self._dequeue_and_enqueue_target_c4(
                            target.repeat_interleave(
                                proj_1_ng_c4.shape[2] * proj_1_ng_c4.shape[3]
                            )
                        )
                    self._dequeue_and_enqueue_c5(
                        proj_1_ng_c5.permute(0, 2, 3, 1).reshape(
                            -1, proj_1_ng_c5.shape[1]
                        )
                    )  # proj_1_ng_c5.shape[1] is channel dimension, the resultant shape is (bs*S^2, C)
                    if self.dense_branch_class_aware:
                        self._dequeue_and_enqueue_target_c5(
                            target.repeat_interleave(
                                proj_1_ng_c5.shape[2] * proj_1_ng_c5.shape[3]
                            )
                        )
                else:
                    loss_dense = pixpro_densecl_loss(
                        pred_1,
                        proj_2_ng,
                        feat_1,
                        feat_2_ng,
                    ...
                    self._dequeue_and_enqueue(
                        proj_1_ng.permute(0, 2, 3, 1).reshape(-1, proj_1_ng.shape[1])
                    )  # proj_1_ng.shape[1] is channel dimension, the resultant shape is (bs*S^2, C)
                    if self.dense_branch_class_aware:
                        self._dequeue_and_enqueue_target(
                            target.repeat_interleave(
                                proj_1_ng.shape[2] * proj_1_ng.shape[3]
                            )
                        )

            elif self.dense_loss_func == "PixPro":
                if self.head_type == "c4c5":
                    loss_dense = 0.5 * (
                        pixpro_loss(
                            pred_1_c4,
