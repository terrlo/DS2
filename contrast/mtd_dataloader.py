...
import cv2
import glob
from contrast.mtd_normal_split import normal_train_imgs, normal_test_imgs

anomaly_types = ["MT_Blowhole", "MT_Break", "MT_Crack", "MT_Fray", "MT_Uneven"]


class MTDTestDataset(Dataset):
    def __init__(self, resize_shape=None):
        self.root_dir = "./dataset/MTD/"
        self.raw_images = []
        self.raw_images.extend(normal_test_imgs)
        for ano_type in anomaly_types:
            self.raw_images.extend(
                sorted(glob.glob(self.root_dir + ano_type + "/Imgs/*.jpg"))
            )
        self.gt_images = [item.split(".jpg")[0] + ".png" for item in self.raw_images]
    ...
    def __getitem__(self, idx):
        ...
        has_anomaly = np.array(
            [0 if "MT_Free" in raw_img_path else 1], dtype=np.float32
        )
...
class MTDTrainDataset(Dataset):
    def __init__(self, resize_shape=None):
        self.resize_shape = resize_shape
        self.image_paths = normal_train_imgs
