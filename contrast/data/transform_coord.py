    ...
    def __call__(self, img, coord=None):
        ...
        for t in self.transforms:
            if "RandomResizedCropCoord" in t.__class__.__name__:
                ...
                # for other augmentations, no "coord" is required
                img = t(img)
        return img, coord
...
class RandomHorizontalFlipCoord(object):
    ...
    def __call__(self, img, coord):
        ...
        if random.random() < self.p:
            ...
            coord_new[0] = coord[2]
            coord_new[2] = coord[0]
            return img.transpose(Image.Transpose.FLIP_LEFT_RIGHT), coord_new
        return img, coord
...
class RandomResizedCropCoord(object):
    ...
    def __init__(
        ...
        self.scale = scale
        self.ratio = ratio
        self.definitive_scale = definitive_scale

    @staticmethod
    def get_params(img, scale, ratio, definitive_scale=None):
        ...
        for attempt in range(10):
            if definitive_scale is None:
                target_area = random.uniform(*scale) * area
            else:
                target_area = definitive_scale * area
    ...
    def __call__(self, img):
        ...
        i, j, h, w, height, width = self.get_params(
            img, self.scale, self.ratio, definitive_scale=self.definitive_scale
        )
        coord = torch.Tensor(
            [
                float(j) / (width - 1),
